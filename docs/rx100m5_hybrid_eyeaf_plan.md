...
## Open question #7 test plan: debug-counter patch (design finalized 2026-08-05)

Purpose: settle, empirically, whether `executeAutoFocusStartTrigger` (the
function the whole trigger-override patch above hooks) is actually reached on
a **physical** S1 half-press in AF-C mode, or only from a tethered/remote
Android-side client — see the "still needed to fully close this out" note
above. This is a pure observation patch: it changes **no behavior**, only
...
2. Round-trip it through `patch/patch_partition.py` (in-place ext2 byte
   ...
   `../patch/README.md`. Explicit user go-ahead required immediately before
   the real flash, per repo policy.
...
- Counter stayed at 0 (or didn't advance during the physical-only test):
  ...
  originally scoped, per the "still needed to fully close this out" note
  above).
